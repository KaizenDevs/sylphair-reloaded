angular.module('edit-mechanic-profile-controller', []);
angular.module('edit-mechanic-profile-controller')
  .controller('EditMechanicCtrl', function (
    $scope,
    simpleStorage,
    simpleSave,
    localStorageService,
    ...
    $scope.createMechanic = simpleStorage.getter($scope.currentKey);
    $scope.showCancelEdit = true;
    $scope.cancelEdit = function () {
      localStorageService.remove($scope.currentKey)
      $location.path('app-mechanic/my-profile/' + localStorageService.get('user_data').data.crew_member.id)
    }

    if ($scope.createMechanic == "") {
      $ionicLoading.show();
      $http({
        url: CONFIG.COMPLETE_API_URL + '/crew_members/edit_attributes'
      }).then(function (response) {
        $scope.createMechanic = response.data;
        $scope.createMechanic = sendProfileObject.addArrays($scope.createMechanic);
        localStorageService.set($scope.currentKey, $scope.createMechanic);
        $ionicLoading.hide();
      }, function (response) {
    ...
    function validateProfile() {
      var checkForm = MechanicService.profile($scope.createMechanic);
      if (checkForm.flag) {
        $ionicPopup.alert({
          title: 'Something is missing!',
          template: 'You should complete the following information to go the next step.' + checkForm.elementsMissing
        });
      } else {
    ...
    function validatePassport() {
      var checkForm = MechanicService.passport($scope.createMechanic);
      if (checkForm.flag) {
        $ionicPopup.alert({
          title: 'Something is missing!',
    ...
    function validateVisa() {
      var checkForm = MechanicService.visa($scope.createMechanic);
      if (checkForm.flag) {
        $ionicPopup.alert({
          title: 'Something is missing!',
          template: checkForm.msg
    ...
    function validateAvailability() {
      var checkForm = MechanicService.availability($scope.createMechanic);
      if (checkForm.flag) {
        $ionicPopup.alert({
          title: 'Something is missing!',
          template: checkForm.msg
